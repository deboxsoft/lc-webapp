<script lang="ts">
  import type { Transaction } from "@deboxsoft/accounting-api";
  import TableItem from "./Item.svelte";
  import {getTransactionContext} from "__@modules/transaksi"

  const {transactionStore} = getTransactionContext();
</script>

<table class="table table-togglable table-hover datatable-responsive-row-control dtr-column dataTable" role="grid">
  <thead>
    <tr role="row">
      <th class="text-center control sorting_disabled d-table-cell" style="width: 20px;">#</th>
      <th class="d-none d-lg-table-cell" style="width: 100px;">No</th>
      <th class="d-none d-xl-table-cell" style="width: 100px;">No Transaksi</th>
      <th class="text-center" style="width: 100px">Tanggal</th>
      <th>Deskripsi</th>
      <th class="text-center" style="width: 200px;">Total</th>
    </tr>
  </thead>
  <tbody>
    {#each $transactionStore as transaction}
      <TableItem expanded={false} journal={transaction} />
    {/each}
  </tbody>
</table>
