...
<script>
  ...
  const { containerWidth, containerHeight, padding } = getContextChart();
</script>

<svg
  bind:this={element}
  class="chart-layout-svg"
  {viewBox}
...
>
  ...
  <g bind:this={innerElement} class="layercake-layout-svg_g" transform="translate({$padding.left}, {$padding.top})">
    <slot {element} />
  </g>
</svg>
