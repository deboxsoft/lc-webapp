<script>
  ...
  import { getSubItemsContext } from "__@stores/subItemsContext";

  export let id = generateId({ prefix: "item", size: 3 });
  export let item;
  export let index;

  const itemContext = getSubItemsContext();
...
<tr>
  ...
  <td class="fit">
    <InputRp id="{id}-amount" class="form-control" name="amount" on:input={amountHandler()} placeholder="Nominal *" />
  </td>
